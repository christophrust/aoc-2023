...
fn main() {
    ...
    include_bytes!("../input.txt")
        ...
        .for_each(|x| {
            ...
            match d.next() {
                Some([10]) => {} // ignore new line character at end of file
                Some([foc]) => {
        ...
        .map(|b| {
            ...
            if !b.slots.is_empty() {
                cnt * b
                    .slots
                    .iter()
                    .fold(0, |a, (_, (f, p))| a + *f as u32 * (*p as u32 + 1))
            } else {
                0
            }
        })
...
fn hash(x: &[u8]) -> u8 {
    x.iter()
        .filter(|&&x| x != 10) // ignore new line character at end of file
        .fold(0, |a, i| {
...
impl LenseBox {
    fn new() -> Self {
        Self {
            slots: HashMap::<Vec<u8>, (u8, usize)>::new(),
        }
    }
}
